#!/usr/bin/env bash
# AZIMUTH validation gate — runs the 4 CLAUDE.md validation checks.
...
TYPOS=$(git ls-files '*.md' 2>/dev/null | grep -vE '^(CHANGELOG|CLAUDE)\.md$' | xargs grep -l precommitment 2>/dev/null | head -3 | tr '\n' ' ')
if [ -n "$TYPOS" ]; then
